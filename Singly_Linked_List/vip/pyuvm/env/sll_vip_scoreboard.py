"""
Singly Linked List VIP Scoreboard
Create Date: 01/05/2026

Scoreboard for Singly Linked List VIP - self-checking component with list model
"""
...
class SllVipScoreboard(uvm_scoreboard):
    """Scoreboard for Singly Linked List VIP with reference model"""
    ...
    def write(self, item):
        ...
        if item.op == SllOp.INSERT_AT_ADDR:
            self.check_insert_at_addr(item)
        elif item.op == SllOp.INSERT_AT_INDEX:
            self.check_insert_at_index(item)
        elif item.op == SllOp.READ_ADDR:
            self.check_read_addr(item)
        elif item.op == SllOp.DELETE_VALUE:
            self.check_delete_value(item)
        elif item.op == SllOp.DELETE_AT_ADDR:
            self.check_delete_at_addr(item)
        elif item.op == SllOp.DELETE_AT_INDEX:
            self.check_delete_at_index(item)
        elif item.op == SllOp.IDLE:
